...
export class PraanService {
  ...
  constructor(private praanRepository: PraanRepository) {}
  getHello(): string {
  ...
  async uploadFile(file: any): Promise<CommonResponseDto> {
    ...
    let uint8Array = new Uint8Array(file.buffer).reduce(
  ...
  async processFile(fileId: number): Promise<CommonResponseDto> {
    ...
    let saveCsv = csv()
      ...
      .subscribe(
        ...
        async (): Promise<Converter> =>
          await this.praanRepository.insertIndividualRecord(
            fileId,
    ...
    let latestFileData =
      ...
      (await this.praanRepository.getFile(fileId));
  ...
  async fetchProcessedFilesMetadata(): Promise<any> {
    let processedFiles =
      await this.praanRepository.fetchProcessedFilesMetadata();
    if (processedFiles.success === false || processedFiles?.length < 1) {
      return {
        success: false,
  ...
  async fetchFileVisualData(fileId: number): Promise<any> {
    let fileData = await this.praanRepository.fetchFileVisualData(fileId);
    if (
      ...
      return {
        success: false,
        message: `No such file found with fileId ${fileId}`,
      };
    }
    return {
      success: true,
      message: 'Record fetched successfully',
      data: { ...fileData, fileDataJSON: JSON.parse(fileData.fileDataJSON) },
    };
  }
}
